...
public eliminarComentario(idPropiedad: number, usuario: string, comentario: any): Observable<any> {
  const token = this.facadeService.getSessionToken();
  const headers = new HttpHeaders({
    'Content-Type': 'application/json',
    Authorization: `Bearer ${token}`,
  ...
  // Asegúrate de que se está enviando el nombre del usuario y el comentario correcto
  return this.http.delete<any>(
    `${environment.url_api}/propiedades/${idPropiedad}/comentarios/eliminar/${usuario}/${comentario.texto}/`,
    { headers }
  );
}
...
obtenerCoordenadas(id: number): Observable<any> {
  const url = `${environment.url_api}/obtener-coordenadas/<int:id>/`;  // Construimos la URL con el ID de la propiedad
  return this.http.get<any>(url);      // Realizamos la solicitud GET
}
}
