...
eliminarComentario(comentario: any): void {
  // Verificar si el comentario pertenece al usuario actual antes de eliminarlo
  if (comentario.usuario === this.nombreUsuario) {
    this.propiedadService.eliminarComentario(this.propiedadSeleccionada.id, this.nombreUsuario, comentario).subscribe({
      next: (response) => {
        console.log('Comentario eliminado con éxito:', response);
        // Eliminar el comentario de la lista local
        const index = this.propiedadSeleccionada.comentarios.findIndex(
          (comentarioItem) => comentarioItem === comentario
        );
        if (index !== -1) {
          this.propiedadSeleccionada.comentarios.splice(index, 1);
        }
      },
      error: (error) => {
    ...
    alert('No puedes eliminar un comentario que no es tuyo.');
  }
}
